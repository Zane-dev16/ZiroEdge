...
actor PersistenceController {
    ...
    private let recoveryPendingURL: URL?
    ...
    // BATCH-04: pending tail for per-token durability without O(n) full copies
    private var pendingTokensBuffer: [String] = []

    // BATCH-04: journal write counting for hot-path verification (≤100 writes per 2k tokens)
    private var journalWriteCount = 0
    private var journalWriteHook: (@Sendable () -> Void)?
    ...
    private init(configuration: PersistenceConfiguration, faultInjector: any PersistenceFaultInjecting) {
        ...
        if let storeURL = configuration.storeURL {
            ...
            self.recoveryPendingURL = storeURL.deletingLastPathComponent()
                .appendingPathComponent(".\(storeURL.lastPathComponent).stream-recovery.pending.json")
        } else {
...
extension PersistenceController {
    ...
    func beginStreamingMessageResult(conversationID: UUID) -> Result<UUID, PersistenceFailure> {
        ...
        do {
            ...
            if let pendingURL = recoveryPendingURL {
    ...
    /// BATCH-04: journal persistence is batched (every flushTokenCount or flushIntervalMs) to avoid O(n) per-token file copies.
    @discardableResult
    func bufferTokens(messageID: UUID, tokens: String) -> Result<Void, PersistenceMutationError> {
        ...
        // Update in-memory state immediately; durable journal + Core Data flush are batched.
        recoveryJournal = journal
        tokenBuffer[messageID] = target
        pendingTokensBuffer.append(tokens)
    ...
    private func finalizeStreamingMessage(
        ...
        tokenBuffer[messageID] = target
        pendingTokensBuffer.removeAll()
        if let pendingURL = recoveryPendingURL { try? FileManager.default.removeItem(at: pendingURL) }
    ...
    func flushPendingWrites() -> [UUID: PersistenceFailure] {
        guard let messageID = recoveryJournal?.messageID, let journal = recoveryJournal else { return [:] }
        // BATCH-04: persist any batched journal content before Core Data flush
        if bufferFlushCount[messageID, default: 0] > 0 {
            do {
                try persistRecoveryJournal(journal)
            } catch {
                return [messageID: .map(error, operation: .journalWrite)]
            }
            bufferFlushCount[messageID] = 0
            lastFlushTime[messageID] = currentTimeMs()
            pendingTokensBuffer.removeAll()
    ...
    func restoreRecoveryJournal() -> Result<Void, PersistenceFailure> {
        ...
        // BATCH-04: merge pending tail (bounded per-token durability) if present
        let pending = loadPendingTokens()
        if !pending.isEmpty {
            let combined = journal.targetContent + pending.joined()
            if combined.utf8.count <= maximumBufferedBytes {
                let merged = RecoveryJournal(
                    messageID: journal.messageID,
                    conversationID: journal.conversationID,
                    createdAt: journal.createdAt,
                ...
                recoveryJournal = merged
                tokenBuffer[journal.messageID] = combined
    ...
    private func persistRecoveryJournal(_ journal: RecoveryJournal) throws {
        ...
        guard let url = recoveryJournalURL else {
            journalWriteCount += 1
            journalWriteHook?()
            return
        }
    ...
    private func persistPendingTokens() throws {
        guard let pendingURL = recoveryPendingURL else { return }
        try FileManager.default.createDirectory(at: pendingURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys]
        try encoder.encode(pendingTokensBuffer).write(to: pendingURL, options: [.atomic])
    }

    private func loadPendingTokens() -> [String] {
        guard let pendingURL = recoveryPendingURL,
              FileManager.default.fileExists(atPath: pendingURL.path),
              let data = try? Data(contentsOf: pendingURL) else { return [] }
        return (try? JSONDecoder().decode([String].self, from: data)) ?? []
    }

    // MARK: - BATCH-04 Journal Write Observability

    func setJournalWriteHook(_ hook: (@Sendable () -> Void)?) {
        journalWriteHook = hook
    }

    func getJournalWriteCount() -> Int { journalWriteCount }

    func resetJournalWriteCount() { journalWriteCount = 0 }
