...
actor ChatSessionActor {
    ...
    private func start(
        ...
        // A holder that will not release surfaces `generationBusy` instead of
        // silently FIFO-queueing this chat behind an unknown decode.
        do {
            try await inferenceService.ensureIdleForNewChat()
        } catch {
    ...
    private func pumpTokens(
        ...
        for try await token in stream {
            ...
            tokenBatch += token
            // Count generated tokens, not flush batches: every yielded chunk
        ...
        if !tokenBatch.isEmpty {
            let tail = tokenBatch
            await MainActor.run { onToken(tail) }
        }
        return true
    }
    ...
    func exportRecovery() async -> Result<Data, PersistenceFailure> {
        ...
        let result = await persistence.exportPartialResponse(recoveryHandle)
        // Export does not consume the recovery (Retry/Discard stay available);
        // only a `.notFound` releases the handle below.
        if case .failure(let failure) = result, failure.category == .notFound {
            recoveryConsumedElsewhere()
        }
        return result
    }
    ...
    /// A `.notFound` result means the journal or its message row no longer
    /// exists — consumed elsewhere (e.g. the conversation was deleted, which
    /// drops the recovery journal with it). The handle must be released or
    /// `start()` would refuse every later stream (`recoveryBufferFull`)
    /// until relaunch, and the "Response not saved yet" banner would wedge
    /// on every surface.
    private func recoveryConsumedElsewhere() {
        recoveryHandle = nil
    }
    ...
    /// Tokens generated by the session, counted one per streamed yield.
