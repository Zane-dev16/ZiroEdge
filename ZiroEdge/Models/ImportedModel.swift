...
struct HFRepositoryInspector: Sendable {
    ...
    init(loader: @escaping Loader = HFRepositoryInspector.liveLoader) {
        self.loader = loader
    }
    ...
    func inspect(_ input: String) async throws -> HFRepositoryReview {
        ...
        let fallbackArchitecture = (topGGUF?["architecture"] as? String)
            ?? Self.architectureHint(repositoryID)
        ...
        for sibling in siblings {
            ...
            let architecture = isProjector ? "clip" : fallbackArchitecture
    ...
    private static func architectureHint(_ repositoryID: String) -> String {
        let value = repositoryID.lowercased()
        if value.contains("gemma") { return "gemma" }
        if value.contains("qwen") && value.contains("vl") { return "qwen2vl" }
        if value.contains("qwen") { return "qwen2" }
        if value.contains("phi") { return "phi3" }
        if value.contains("mistral") { return "mistral" }
        return "llama"
    }
