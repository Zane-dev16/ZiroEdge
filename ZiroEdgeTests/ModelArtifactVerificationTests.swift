...
extension ModelArtifactVerificationTests {
    ...
    func testDownloadRefusedForModelWithInvalidCatalogMetadata() {
        ...
        // Hermetic disk space: the storage gate runs before the catalog gate
        // and reads the real host volume, so on a nearly-full disk this test
        // would observe diskSpaceInsufficient instead of the catalog error
        // under test. Injecting plentiful space isolates the catalog refusal.
...
extension ModelArtifactVerificationTests {
    ...
    func testVerificationFailureDoesNotPromoteOrCorruptInstalled() throws {
        ...
        // Hermetic disk space: verifyAndPromote refuses to promote (and
        // deliberately preserves staging for resume) when the real host
        // volume is below the safety margin. Injecting plentiful space makes
        // the SHA-mismatch path under test deterministic.
        let result = DownloadManager(availableDiskSpaceProvider: { .max }).verifyAndPromote(task: task)
