...
final class ImportStoragePreflightTests: XCTestCase {
    ...
    func testRequiredDownloadBytesExcludesAlreadyInstalledArtifacts() {
        ...
        let hash = TestModelFixtures.sha256(gguf)
        // Create a model whose base SHA matches the installed artifact.
        var installed = model
        // We need a model with the same baseArtifactStorageID and SHA.
